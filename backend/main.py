...
# Conversation states
BAN_ID, UNBAN_ID, BALANCE_ID, BALANCE_AMOUNT, RESET_ID, RESET_PASS, DEDUCT_ID, DEDUCT_AMOUNT = range(8)
ADD_USER_USERNAME, ADD_USER_PASSWORD, ADD_USER_UPI, ADD_USER_EMAIL, ADD_USER_USDT = range(8, 13)
...
# 4. DEDUCT BALANCE
async def deduct_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update):
        await update.message.reply_text("⛔ Unauthorized.")
        return ConversationHandler.END
    await update.message.reply_text("👤 Send me the User ID to deduct balance from.")
    return DEDUCT_ID

async def deduct_get_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.text.strip()
    user = await get_user_by_id(user_id)
    if not user:
        await update.message.reply_text("❌ User not found.")
        return ConversationHandler.END
    context.user_data['deduct_id'] = user_id
    await update.message.reply_text("💰 Send me the amount (in ₹) to deduct.")
    return DEDUCT_AMOUNT

async def deduct_get_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        amount = int(update.message.text.strip())
        user_id = context.user_data.get('deduct_id')
        user = await get_user_by_id(user_id)
        if not user:
            await update.message.reply_text("❌ User not found.")
            return ConversationHandler.END
        ...
        if current < amount:
            await update.message.reply_text(f"❌ Insufficient balance. User has ₹{current}.")
            return ConversationHandler.END
        new_balance = current - amount
        await update_user(user_id, {'balance': new_balance})
        await update.message.reply_text(f"✅ Deducted ₹{amount} from user {user_id}. New balance: ₹{new_balance}.")
    except ValueError:
        await update.message.reply_text("❌ Please send a valid number.")
        return DEDUCT_AMOUNT
    return ConversationHandler.END
...
async def add_user_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update):
        await update.message.reply_text("⛔ Unauthorized.")
        return ConversationHandler.END
    await update.message.reply_text("👤 Enter the username for the new user:")
...
async def add_user_username(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if existing:
        await update.message.reply_text("❌ Username already exists. Please choose another.")
        return ADD_USER_USERNAME
...
async def add_user_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("💳 Enter the UPI ID for the new user:")
    return ADD_USER_UPI

async def add_user_upi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    upi = update.message.text.strip()
    context.user_data['new_upi'] = upi
    await update.message.reply_text("📧 Enter the email address for the new user:")
...
async def add_user_email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("💵 Enter the USDT address (optional, or type 'none' to skip):")
    return ADD_USER_USDT

async def add_user_usdt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    usdt = update.message.text.strip()
    if usdt.lower() == 'none':
        usdt = ''
    context.user_data['new_usdt'] = usdt
    ...
    try:
        ...
        await bot.send_message(chat_id=ADMIN_ID, text=f"MANUAL ADD: {user_id} ({username})")
    except:
        pass
    return ConversationHandler.END
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        return
    await update.message.reply_text("🤖 Sunshine Admin Bot\nSend /help to see all commands.")
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "/deduct_balance - Deduct money from a user (bot will ask for ID and amount)\n"
        "/reset_password - Reset user password (bot will ask for ID and new pass)\n"
        "/add_user - Manually create a new user (bot will ask for details)\n"
...
async def startup():
    ...
    conv_deduct = ConversationHandler(
        entry_points=[CommandHandler("deduct_balance", deduct_start)],
        states={
            DEDUCT_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, deduct_get_id)],
            DEDUCT_AMOUNT: [MessageHandler(filters.TEXT & ~filters.COMMAND, deduct_get_amount)]
        },
        fallbacks=[CommandHandler("cancel", cancel)]
    )
    ...
    conv_add_user = ConversationHandler(
        ...
        states={
            ...
            ADD_USER_UPI: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_user_upi)],
            ADD_USER_EMAIL: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_user_email)],
            ADD_USER_USDT: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_user_usdt)],
        },
        fallbacks=[CommandHandler("cancel", cancel)]
    )
    ...
    bot_app.add_handler(conv_deduct)
    bot_app.add_handler(conv_reset)
    bot_app.add_handler(conv_add_user)
